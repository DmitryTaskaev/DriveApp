...
function GiveBan($connect, $key, $id){
    if(checkFullInfoForAccount($connect, $key, 'GiveBan')){
        $l = mysqli_real_escape_string($connect, $id);
        $post = mysqli_query($connect, "SELECT * FROM `account` WHERE `Login` = '$l'");
        $user = mysqli_fetch_assoc($post);
        if(mysqli_num_rows($post) === 0) {
            ...
            if($user['Ban'] == 0) {
                $post = mysqli_query($connect, "UPDATE `account` SET `Ban` = 1 WHERE `Login` = '$l'");
                $res = [
                    "status" => false,
                    "code" => "0005",
                    "result" => "Наказание выдано"
                ];
                echo json_encode($res);
            }
            else {
                $post = mysqli_query($connect, "UPDATE `account` SET `Ban` = 0 WHERE `Login` = '$l'");
                $res = [
                    "status" => false,
                    "code" => "0005",
                    "result" => "Наказание снято"
                ];
                echo json_encode($res);
            }
...
function GiveWarn($connect, $key, $id){
    if(checkFullInfoForAccount($connect, $key, 'GiveWarn')){
        $l = mysqli_real_escape_string($connect, $id);
        $post = mysqli_query($connect, "SELECT * FROM `account` WHERE `Login` = '$l'");
        $user = mysqli_fetch_assoc($post);
        if(mysqli_num_rows($post) === 0) {
            ...
            $temp = $user['Warn'] + 1;
            if($temp == 4){
                $posts = mysqli_query($connect, "UPDATE `account` SET `Ban` = 1 WHERE `Login` = '$l'");
                $res = [
                    "status" => false,
                    "code" => "0005",
                    "result" => "Выдан бан из-за большого количества нарушений"
                ];
                echo json_encode($res);
            }
            else {
                $t = $user['Warn'] + 1;
                $posts = mysqli_query($connect, "UPDATE `account` SET `Warn` = $t WHERE `Login` = '$l'");
                echo json_encode($posts);
                $res = [
                    "status" => true,
                    "code" => "0005",
                    "result" => "Выдано наказание"
                ];
                echo json_encode($res);
            }
